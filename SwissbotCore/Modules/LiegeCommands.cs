...
namespace Official_Bot.Modules
{
    ...
    {
        [DiscordCommand("winner")]
        public async Task AddEventWinner(string userRaw)
        {
            var user = await GetUser(userRaw);
            var role = Context.Guild.GetRole(829483909459869696);

            if (role == null)
                return;

            if (Context.User is SocketGuildUser executer)
            {
                if (!executer.Roles.Any(x => x.Id == 779943693951565824))
                {
                ...
                if (user is null)
                {
                    ...
                    {
                        Title = "Error",
                        Description = $"Please provide a user! The argument \"{userRaw}\" was not able to be parsed as a user.",
                        Color = Color.Red,
                    }.WithCurrentTimestamp().Build());
                    return;
                }
                ...
                {
                    Title = "Success",
                    Description = $"Gave {user.Mention} the {role.Mention} role!",
                    Color = Color.Green,
                }.WithCurrentTimestamp().Build());
                return;
            }
            else
            {
                ...
                {
                    Title = "Error",
                    Description = $"Liege's bad casting failed.",
                    Color = Color.Red,
                }.WithCurrentTimestamp().Build());
                return;
            }
